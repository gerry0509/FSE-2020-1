...
El firmware está basado en la librería Keyboard.c de Microchip Solutions
...
static void InitializeSystem(void)
{
        ...
        ADCON1 = 0x00;                 // Default all pins to digital
...
void UserInit(void)
{
    ...
    // transmission
	OpenADC(ADC_FOSC_16 & ADC_RIGHT_JUST & ADC_4_TAD,ADC_INT_OFF & ADC_VREFPLUS_VDD & ADC_VREFMINUS_VSS, ADC_12ANA);
	...
	USART_EIGHT_BIT &
	USART_CONT_RX &
	USART_BRGH_HIGH, 155);	// 19200 baudios
...
void Led(int estado) {
	...
	PORTBbits.RB1=estado;	
}	

int MtouchKey(UINT8 Canal) {
	switch (Canal) { // ponemos el canal analógico a masa
		case 0:
			TRISAbits.TRISA1 = 0; 
			PORTAbits.RA1 = 0; 
			break;
	...
	TRISAbits.TRISA0  = 0;     // Ahora cargamos a 5V el canal analógico
	PORTAbits.RA0  = 1;    
	SetChanADC(ADC_CH0); 
	
	switch (Canal) { // seleccionamos el canal solicitado como entrada analógica
		case 0:
			TRISAbits.TRISA1 = 1; 
			SetChanADC(ADC_CH1); 
			break;
	...
	Delay10TCYx(5); 	// pequeña pausa para cargar condensador
	ConvertADC();		// comenzamos la conversión
	while (BusyADC());	// y esperamos a que termine
	return (ReadADC());	// para devolver el resultado
}	
void MtouchCVD() {

#define SENSIBILIDAD	200
	...
	RE = MtouchKey(0);
	if ((ADC[0]-RE)>SENSIBILIDAD)
		ClaveHID=ClaveHID | 0x01;
	ADC[0]=(ADC[0]+RE)/2;
	
	RE = MtouchKey(1);
	if ((ADC[1]-RE)>SENSIBILIDAD)
		ClaveHID=ClaveHID | 0x08;
	ADC[1]=(ADC[1]+RE)/2;	

	RE = MtouchKey(2);
	if ((ADC[2]-RE)>SENSIBILIDAD)
		ClaveHID=ClaveHID | 0x02;
	ADC[2]=(ADC[2]+RE)/2;	
	
	RE = MtouchKey(3);
	if ((ADC[3]-RE)>SENSIBILIDAD)
		ClaveHID=ClaveHID | 0x04;
	ADC[3]=(ADC[3]+RE)/2;	

	RE = MtouchKey(4);
	if ((ADC[4]-RE)>SENSIBILIDAD)
		ClaveHID=ClaveHID | 0x10;
	ADC[4]=(ADC[4]+RE)/2;	

	RE = MtouchKey(5);
	if ((ADC[5]-RE)>SENSIBILIDAD)
		ClaveHID=ClaveHID | 0x40;
	ADC[5]=(ADC[5]+RE)/2;	
	
	RE = MtouchKey(6);
	if ((ADC[6]-RE)>SENSIBILIDAD)
		ClaveHID=ClaveHID | 0x20;
	ADC[6]=(ADC[6]+RE)/2;	
   ...
   if(!HIDTxHandleBusy(lastINTransmission))
    {
		if(key_pass==0){
			if (!ClaveHID) return;
			Led(1);			
			hid_report_in[1]=ClaveHID;
			hid_report_in[0]=2;			
			key_pass=1;					// evita la autorrepetición
